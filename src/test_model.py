...
if __name__ == '__main__':
    #give test prompt
    prompt = "Multiply both sides by x: A/x = C/B -->"
    ...
    #load model
    model = torch.load('models/basic-math-final.pth',map_location =device)
    #define criterion and optimizer
    criterion = nn.CrossEntropyLoss(ignore_index=tokenizer.pad_token_id)
    optimizer = torch.optim.Adam(model.parameters())
    #response,_ = generate_with_gumbel(prompt, model,tokenizer,device, temperature=1.3)
    _,response,_ = train_on_new_problem(model,criterion,optimizer,epochs=1000,model_name='test-new-problem',problem=prompt,in_temperature=1.5,tokenizer=tokenizer,device=device)
    print(response)
